...
cat certs/nginxdemo.net.crt.pem certs/nginxdemo.net.issuer.pem >certs/nginxdemo.net.fullchain.pem
...
sudo docker run -d   --restart=always   --name registryY   -v "$(pwd)"/certs:/certs   -e REGISTRY_HTTP_ADDR=0.0.0.0:443 -e REGISTRY_HTTP_TLS_CERTIFICATE=certs/nginxdemo.net.fullchain.pem   -e REGISTRY_HTTP_TLS_KEY=certs/nginxdemo.net.key.pem  -p 443:443  registry:2
...
#sudo sh -c 'echo "127.0.0.1 dockerregistry.nginxdemo.net">>/etc/hosts'

sudo docker tag $NPLUS_IMAGE dockerregistry.nginxdemo.net/nginxplus
sudo docker push dockerregistry.nginxdemo.net/nginxplus
